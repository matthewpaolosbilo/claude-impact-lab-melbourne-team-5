...
> Seven tasks ✅ DONE: 3.1 (Vite + React 19 + Tailwind v4 with brand tokens), 3.2 (`api.js` axios instance + `.env.example`), 3.3 (design tokens applied), 3.4 (`App.jsx` router with `/` and `/profile` placeholder routes, branch `feat-3.4`), 3.5 (`NavHeader.jsx` global sticky top bar, branch `feat-3.5`), 3.7 (`Home.jsx` layout with map + slots + FAB, branch `feat-3.7`), 3.13 (vite proxy `/api` → `localhost:8000`). The frontend shell is now visible and routable end-to-end; remaining work hinges on Dev 1's events/users endpoints (3.6, 3.8, 3.9, 3.10). Dev 3 is the integration hub: it owns three shared files (`api.js`, `App.jsx`, `EventCard.jsx`) and the Netlify deploy tail.
...
With 3.5 and 3.7 ✅ shipped (on top of the earlier 3.3 and 3.4), the router and layout branches collapse. The longest unavoidable chain is now:

`3.8 → 3.9 → 3.10 → 3.15` (four tasks, all downstream of EventCard and gated on Dev 1's events/RSVP endpoints).

The auth-branch chain `3.6 → 3.10 → 3.15` is three tasks. 3.10 remains the fan-in, 3.15 the final gate.

3.12 (mobile) and 3.11 (empty states) also block 3.15 but are independent leaves of similar depth and can no longer be started in isolation because they need real cards (3.8) to be meaningful.
